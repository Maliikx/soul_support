  ...
  try {
    final response = await http.post(
      ...
      body: json.encode({
        ...
        "doctor_id": doctorId,
        "Amount": amount, // ensure spelling matches the backend model
        "Date": date,
      }),
    );
...
class _ConfirmappointmentState extends State<Confirmappointment> {
  ...
  Future<void> _loadEmail() async {
    ...
    _id = await patient.getpatientId(_email!);
    _docId = await doctorid.getdoctorId(_name!);

    print('Email from Hive: $_email');
    print('patient from Hive: $_id');
    print('name from Hive: $_name');
    print('doctor_id: $_docId');
