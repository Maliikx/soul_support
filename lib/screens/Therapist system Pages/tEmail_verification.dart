...
import 'package:soul_support/screens/Therapist%20system%20Pages/TherapistWelcomePhotoScreen.dart';
import 'dart:convert';
import 'dart:async';

import 'package:soul_support/screens/Therapist%20system%20Pages/therapistWelcomeScreen.dart';
...
class TEmailVerificationScreen extends StatefulWidget {
  final String email;
  final VerificationType verificationType;

  const TEmailVerificationScreen({
    super.key,
    required this.email,
  ...
  @override
  State<TEmailVerificationScreen> createState() => _TEmailVerificationScreen();
}

class _TEmailVerificationScreen extends State<TEmailVerificationScreen> {
  ...
  Future<void> _resendCode() async {
    ...
    try {
      final url = Uri.parse("$baseUrl/doctor/resend-otp");
  ...
  Future<void> _verifyCode() async {
    ...
    try {
      final url = Uri.parse("$baseUrl/doctor/verify-otp");
      final response = await http.post(
        url,
        headers: {"Content-Type": "application/json"},
        ...
        if (responseData['success'] == true ||
          ...
          Future.delayed(const Duration(seconds: 1), () {
            Navigator.pushReplacement(
              context,
              MaterialPageRoute(builder: (context) => TherapistWelcomePhotoScreen()),
            );
          });
        } else {
          ScaffoldMessenger.of(context).showSnackBar(
