...
import 'package:soul_support/screens/Therapist%20system%20Pages/tEmail_verification.dart';
import 'dart:convert';
// import 'phone_number_code.dart';

class TVerificationChoiceScreen extends StatefulWidget {
  final String email;
  ...
  const TVerificationChoiceScreen({
    super.key,
    required this.email,
  ...
  @override
  State<TVerificationChoiceScreen> createState() =>
      _TVerificationChoiceScreenState();
}

class _TVerificationChoiceScreenState extends State<TVerificationChoiceScreen> {
  ...
  Future<void> sendOtpAndNavigate() async {
    ...
    try {
      ...
      final response = await http.post(
        ...
        body: jsonEncode({
          "Name": widget.name,
          "Qualification": widget.qualification,  // You need to get this from somewhere
          "email": widget.email,
          "Age": widget.age,                      // Provide age if you have it
          "Rating": widget.rating ?? 0,           // Default if you don't have rating
          "NumberOfPatient": widget.numberOfPatient ?? 0, // Default if needed
          // "Calender": (widget.calender != null && widget.calender != "Invalid date")
          //     ? widget.calender
          //     : null,
          // Default empty if you don't have this
      ...
      if (response.statusCode == 201 || responseData['message'].toString().contains("OTP")) {
        ...
        Future.delayed(const Duration(milliseconds: 500), () {
          Navigator.pushReplacement(
            context,
            MaterialPageRoute(
              builder: (context) => TEmailVerificationScreen(
                email: widget.email,
